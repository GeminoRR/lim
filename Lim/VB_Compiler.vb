...
'                        'New
'                        content.Add(String.Format("Shared Public {0} As New {1}", var.compiledName, compileSafeType(var.type)))
'                    Else
'                        'Normal
'                        content.Add(String.Format("Shared Public {0} As {1} = Nothing", var.compiledName, compileSafeType(var.type)))
'                    End If
...
'                            content.Add(String.Format("Shared Public {0} As {1} = {2}", var.compiledName, compileSafeType(var.type), compileNode(declareVariable.value, content)))

'                        Case VariableDeclarationType._var_
'                            content.Add(String.Format("Shared Public {0} As {1} = {2}.clone()", var.compiledName, compileSafeType(var.type), compileNode(declareVariable.value, content)))
...
'        'F-StringNode
'        If TypeOf node Is FStringNode Then
...
'        ElseIf TypeOf node Is FStringNode Then

'            'Compile
'            Return compileFString(DirectCast(node, FStringNode), content)
...
'    '========== COMPILE F-STRING ==========
'    '======================================
'    Private Function compileFString(ByVal node As FStringNode, ByVal content As List(Of String))

'        'Variables
'        Dim searchVariable As Boolean = False
'        Dim finalString = ""
'        Dim lastSearch = ""
'        Dim exitChar As Boolean = False

'        'TODO: AST
'        'For each char
'        For Each c As Char In node.value

'            If exitChar = True Then
...
'                If searchVariable = False Then
'                    addNodeSyntaxError("VBCFS01", "A ""}"" is used before a ""{""", node)
'                End If
'                Dim var As Variable = getVariable(lastSearch, node)
'                finalString += """ + (" + var.compiledName + ").__str__().value + """
'                Continue For
...
'        'Return
'        Return "New str(""" & node.value.ToString().Replace("""", """""") & """)"
